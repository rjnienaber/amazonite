...
module Amazonite::DynamoDBV2
  ...
  # Optionally, you can also set `Exists` to false, indicating that you *do not* expect to find the
  # attribute value in the table. In this case, the conditional operation succeeds only if the
  ...
  class ExpectedAttributeValue
    ...
    # the table. If it is found, then the operation succeeds. If it is not found, the operation fails
    ...
    # If in fact the value does not exist, then the assumption is valid and the operation succeeds. If
    # the value is found, despite the assumption that it does not exist, the operation fails with a
    # `ConditionCheckFailedException`.
    #
    # The default setting for `Exists` is `true`. If you supply a `Value` all by itself, DynamoDB
    # assumes the attribute exists: You don't have to set `Exists` to `true`, because it is implied.
    #
    # DynamoDB returns a `ValidationException` if:
    #
    # - `Exists` is `true` but there is no `Value` to check. (You expect a value to exist, but don't
    ...
    #
    # `EQ | NE | LE | LT | GE | GT | NOT_NULL | NULL | CONTAINS | NOT_CONTAINS | BEGINS_WITH | IN |
    # BETWEEN`
    #
    ...
    # String Set, Number Set, or Binary Set. If an item contains an `AttributeValue` of a different
    # type than the one provided in the request, the value does not match. For example, `{"S":"6"}`
    # does not equal `{"N":"6"}`. Also, `{"N":"6"}` does not equal `{"NS":["6", "2", "1"]}`.
    #
    ...
    # not equal `{"N":"6"}`. Also, `{"N":"6"}` does not compare to `{"NS":["6", "2", "1"]}`.
    #
    # - `NOT_NULL` : The attribute exists. `NOT_NULL` is supported for all data types, including lists
    # and maps.
    #
    ...
    #
    # - `NULL` : The attribute does not exist. `NULL` is supported for all data types, including lists
    # and maps.
    #
    # This operator tests for the nonexistence of an attribute, not its data type. If the data type of
    # attribute "`a`" is null, and you evaluate it using `NULL`, the result is a Boolean `false`. This
    ...
    #
    # `AttributeValueList` can contain only one `AttributeValue` element of type String, Number, or
    # Binary (not a set type). If the target attribute of the comparison is of type String, then the
    # operator checks for a substring match. If the target attribute of the comparison is of type
    # Binary, then the operator looks for a subsequence of the target that matches the input. If the
    # target attribute of the comparison is a set ("`SS`", "`NS`", or "`BS`"), then the operator
    # evaluates to true if it finds an exact match with any member of the set.
    #
    # CONTAINS is supported for lists: When evaluating "`a CONTAINS b`", "`a`" can be a list; however,
    # "`b`" cannot be a set, a map, or a list.
    #
    # - `NOT_CONTAINS` : Checks for absence of a subsequence, or absence of a value in a set.
    #
    # `AttributeValueList` can contain only one `AttributeValue` element of type String, Number, or
    # Binary (not a set type). If the target attribute of the comparison is a String, then the
    # operator checks for the absence of a substring match. If the target attribute of the comparison
    # is Binary, then the operator checks for the absence of a subsequence of the target that matches
    # the input. If the target attribute of the comparison is a set ("`SS`", "`NS`", or "`BS`"), then
    ...
    # or Binary. These attributes are compared against an existing attribute of an item. If any
    ...
    # Number, or Binary (not a set type). A target attribute matches if the target value is greater
    ...
    # code values. For example, `a` is greater than `A`, and `a` is greater than `B`. For a list of
    # code values, see
    ...
    # Format](https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/DataFormat.html) in the
