...
module Amazonite::CloudFormation
  ...
  class TypeSummary
    ...
    def validate! : Nil
      ...
      if value = @publisher_name
        raise Core::ValidationError.new("PublisherName length must be >= 1") if value.size < 1
        raise Core::ValidationError.new("PublisherName length must be <= 100") if value.size > 100
        raise Core::ValidationError.new("PublisherName does not match the required pattern") unless value.matches?(Regex.new("^[\\s\\S]+$"))
      end
    end
