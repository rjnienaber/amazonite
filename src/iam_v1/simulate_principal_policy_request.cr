...
module Amazonite::IamV1
  class SimulatePrincipalPolicyRequest
    ...
    def validate! : Nil
      ...
      if value = @policy_exclusion_list
        raise Core::ValidationError.new("PolicyExclusionList must have at least 0 item(s)") if value.size < 0
        raise Core::ValidationError.new("PolicyExclusionList must have at most 256 item(s)") if value.size > 256
        value.each(&.validate!)
      end
