...
module Amazonite::DynamoDB
  ...
  class UpdateItemInput
    ...
    # The name of the table containing the item to update. You can also provide the Amazon Resource
    ...
    # append the new element to the start or the end of the list by reversing the order of the
    # operands.
    #
    ...
    # suppose that the item you want to update doesn't have an attribute named `itemcount`, but you
    # decide to `ADD` the number `3` to this attribute anyway. DynamoDB will create the `itemcount`
    ...
    # is specified for a set attribute and the attribute type specified does not match the existing
    # set type.
    #
    ...
    # An optional parameter that returns the item attributes for an `UpdateItem` operation that failed
    # a condition check.
    #
    # There is no additional cost associated with requesting a return value aside from the small
    # network and processing overhead of receiving a larger response. No read capacity units are
    ...
    def_equals_and_hash(@table_name, @key, @attribute_updates, @expected, @conditional_operator, @return_values, @return_consumed_capacity, @return_item_collection_metrics, @update_expression, @condition_expression, @expression_attribute_names, @expression_attribute_values, @return_values_on_condition_check_failure)
  end
end
