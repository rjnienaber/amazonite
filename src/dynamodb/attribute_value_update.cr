...
module Amazonite::DynamoDB
  ...
  class AttributeValueUpdate
    ...
    # example, if the attribute value was the set `[a,b,c]` and the `DELETE` action specified `[a,c]`,
    # then the final attribute value would be `[b]`. Specifying an empty set is an error.
    #
    # - `ADD` - If the attribute does not already exist, then the attribute and its values are added
    # to the item. If the attribute does exist, then the behavior of `ADD` depends on the data type of
    # the attribute:
    #
    ...
    #
    # In addition, if you use `ADD` to update an existing item, and intend to increment or decrement
    # an attribute value which does not yet exist, DynamoDB uses `0` as the initial value. For
    # example, suppose that the item you want to update does not yet have an attribute named
    # *itemcount*, but you decide to `ADD` the number `3` to this attribute anyway, even though it
    # currently does not exist. DynamoDB will create the *itemcount* attribute, set its initial value
    # to `0`, and finally add `3` to it. The result will be a new *itemcount* attribute in the item,
    # with a value of `3`.
    #
    # - If the existing data type is a set, and if the `Value` is also a set, then the `Value` is
    # added to the existing set. (This is a *set* operation, not mathematical addition.) For example,
    # if the attribute value was the set `[1,2]`, and the `ADD` action specified `[3]`, then the final
    ...
    #
    # - `ADD` - DynamoDB creates a new item with the supplied primary key and number (or set) for the
