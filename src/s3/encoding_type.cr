...
module Amazonite::S3
  ...
  enum EncodingType
    ...
    def self.to_json(e : EncodingType, json : JSON::Builder) : Nil
      value = case e
              when AS::EncodingType::Url then "url"
              else
    ...
    def self.from_json(pull : JSON::PullParser) : AS::EncodingType
      value = pull.read_string
      case value
      when "url" then AS::EncodingType::Url
      else
    ...
    def self.from_json_object_key?(key : String) : AS::EncodingType?
      case key
      when "url" then AS::EncodingType::Url
      else
        nil
