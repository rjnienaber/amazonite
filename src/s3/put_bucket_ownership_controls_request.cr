...
module Amazonite::S3
  class PutBucketOwnershipControlsRequest
    ...
    # The `OwnershipControls` (BucketOwnerEnforced, BucketOwnerPreferred, or ObjectWriter) that you
    # want to apply to this Amazon S3 bucket.
    property ownership_controls : OwnershipControls
    ...
    # this header, there must be a corresponding `x-amz-checksum-*algorithm* ` header sent. Otherwise,
    ...
    def validate! : Nil
      if value = @ownership_controls
        value.validate!
      end
    end

    def_equals_and_hash(@bucket, @content_md5, @expected_bucket_owner, @ownership_controls, @checksum_algorithm)
  end
end
