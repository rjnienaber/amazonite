...
module Amazonite::S3
  ...
  class CompletedPart
    ...
    # multipart upload request was created with the `CRC64NVME` checksum algorithm. For more
    ...
    # `x-amz-checksum-sha256`) is applied to each part, the `PartNumber` must start at 1 and the part
    # numbers must be consecutive. Otherwise, Amazon S3 generates an HTTP `400 Bad Request` status
    # code and an `InvalidPartOrder` error code.
    #
    # - **Directory buckets** - In `CompleteMultipartUpload`, the `PartNumber` must start at 1 and the
    ...
    def_equals_and_hash(@e_tag, @checksum_crc32, @checksum_crc32c, @checksum_crc64nvme, @checksum_sha1, @checksum_sha256, @checksum_sha512, @checksum_md5, @checksum_xxhash64, @checksum_xxhash3, @checksum_xxhash128, @part_number)
  end
end
