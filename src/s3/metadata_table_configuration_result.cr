...
module Amazonite::S3
  # The V1 S3 Metadata configuration for a general purpose bucket. The destination table bucket must
  # be in the same Region and Amazon Web Services account as the general purpose bucket. The
  ...
  class MetadataTableConfigurationResult
    ...
    # specified metadata table name must be unique within the `aws_s3_metadata` namespace in the
    # destination table bucket.
    property s3_tables_destination_result : S3TablesDestinationResult
    ...
    def build_xml(xml : XML::Builder) : Nil
      xml.element("S3TablesDestinationResult") { @s3_tables_destination_result.build_xml(xml) }
    end

    def self.from_xml(node : XML::Node) : self
      new(
        s3_tables_destination_result: node.xpath_node("*[local-name()='S3TablesDestinationResult']").try { |n| S3TablesDestinationResult.from_xml(n) }.not_nil!,
      )
    end

    def validate! : Nil
      if value = @s3_tables_destination_result
        value.validate!
      end
    end

    def_equals_and_hash(@s3_tables_destination_result)
  end
end
