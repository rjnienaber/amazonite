private alias AS = Amazonite::Sts
private alias Core = Amazonite::Core

module Amazonite::Sts
  class Client < Core::Client
    Log               = ::Log.for("amazonite.sts.client")
    ...
    # security token. Typically, you use `AssumeRole` within your account or for cross-account access.
    ...
    # Amazon Web Services service with the following exception: You cannot call the Amazon Web
    ...
    # that you use for both inline and managed session policies can't exceed 2,048 characters. Passing
    ...
    # in the *IAM User Guide*.
    #
    # When you create a role, you create two policies: a role trust policy that specifies *who* can
    # assume the role, and a permissions policy that specifies *what* can be done with the role. You
    # specify the trusted principal that is allowed to assume the role in the role trust policy.
    #
    # To assume a role from a different account, your Amazon Web Services account must be trusted by
    # the role. The trust relationship is defined in the role's trust policy when the role is created.
    # That trust policy states which accounts are allowed to delegate that access to users in the
    # account.
    #
    ...
    # trust policy trusts the account).
    #
    ...
    # policies, see [IAM
    # Policies](https://docs.aws.amazon.com/IAM/latest/UserGuide/access_policies.html) in the *IAM
    # User Guide*.
    #
    # **Tags**
    #
    ...
    # tags. For more information, see [Tutorial: Using Tags for Attribute-Based Access
    # Control](https://docs.aws.amazon.com/IAM/latest/UserGuide/tutorial_attribute-based-access-control.html)
    # in the *IAM User Guide*.
    #
    # You can set the session tags as transitive. Transitive tags persist during role chaining. For
    ...
    # Access](https://docs.aws.amazon.com/IAM/latest/UserGuide/MFAProtectedAPI.html) in the *IAM User
    # Guide* guide.
    #
    # To use MFA with `AssumeRole`, you pass values for the `SerialNumber` and `TokenCode` parameters.
    # The `SerialNumber` value identifies the user's hardware or virtual MFA device. The `TokenCode`
    ...
    # store or directory to role-based Amazon Web Services access without user-specific credentials or
    ...
    # apply when you use those operations to create a console URL. For more information, see [Using
    # IAM Roles](https://docs.aws.amazon.com/IAM/latest/UserGuide/id_roles_use.html) in the *IAM User
    # Guide*.
    #
    # [Role
    # chaining](https://docs.aws.amazon.com/IAM/latest/UserGuide/id_roles_terms-and-concepts.html#iam-term-role-chaining)
    # limits your CLI or Amazon Web Services API role session to a maximum of one hour. When you use
    # the `AssumeRole` API operation to assume a role, you can specify the duration of your role
    # session with the `DurationSeconds` parameter. You can specify a parameter value of up to 43200
    # seconds (12 hours), depending on the maximum session duration setting for your role. However, if
    # you assume a role using role chaining and provide a `DurationSeconds` parameter value greater
    ...
    # to any Amazon Web Services service with the following exception: you cannot call the STS
    ...
    # credentials. The identity of the caller is validated by using keys in the metadata document that
    ...
    # about session tags, see [Passing Session Tags in
    # STS](https://docs.aws.amazon.com/IAM/latest/UserGuide/id_session-tags.html) in the *IAM User
    # Guide*.
    #
    # You can pass up to 50 session tags. The plaintext session tag keys can’t exceed 128 characters
    # and the values can’t exceed 256 characters. For these and additional limits, see [IAM and STS
    # Character
    ...
    # provider (IdP) to issue the claims required by Amazon Web Services. Additionally, you must use
    ...
    # with the [Amazon Web Services SDK for iOS Developer Guide](http://aws.amazon.com/sdkforios/) and
    # the [Amazon Web Services SDK for Android Developer Guide](http://aws.amazon.com/sdkforandroid/)
    # to uniquely identify a user. You can also supply the user with a consistent identity throughout
    # the lifetime of an application.
    #
    # To learn more about Amazon Cognito, see [Amazon Cognito identity
    # pools](https://docs.aws.amazon.com/cognito/latest/developerguide/cognito-identity.html) in
    ...
    # requests temporary security credentials without including long-term Amazon Web Services
    # credentials in the application. You also don't need to deploy server-based proxy services that
    # use long-term Amazon Web Services credentials. Instead, the identity of the caller is validated
    ...
    # access key, and a security token. Applications can use these temporary security credentials to
    # sign calls to Amazon Web Services service API operations.
    #
    # **Session Duration**
    #
    ...
    # how to view the maximum value for your role, see [Update the maximum session duration for a role
    ...
    # **Permissions**
    #
    # The temporary security credentials created by `AssumeRoleWithWebIdentity` can be used to make
    ...
    # tags. Each session tag consists of a key name and an associated value. For more information
    # about session tags, see [Passing session tags using
    ...
    #
    # You can pass a session tag with the same key as a tag that is attached to the role. When you do,
    # the session tag overrides the role tag with the same key.
    #
    # An administrator must grant you the permissions necessary to pass session tags. The
    # administrator can also create granular permissions to allow you to pass only specific session
    # tags. For more information, see [Tutorial: Using Tags for Attribute-Based Access
    ...
    # from a supported identity provider and create a role that the application can assume. The role
    ...
    # policy.
    #
    # Calling `AssumeRoleWithWebIdentity` can result in an entry in your CloudTrail logs. The entry
    # includes the [Subject](http://openid.net/specs/openid-connect-core-1_0.html#Claims) of the
    # provided web identity token. We recommend that you avoid using any personally identifiable
    # information (PII) in this field. For example, you could instead use a GUID or a pairwise
    # identifier, as [suggested in the OIDC
    # specification](http://openid.net/specs/openid-connect-core-1_0.html#SubjectIDTypes).
    #
    # For more information about how to use OIDC federation and the `AssumeRoleWithWebIdentity` API,
    # see the following resources:
    #
    # - [Using Web Identity Federation API Operations for Mobile
    # Apps](https://docs.aws.amazon.com/IAM/latest/UserGuide/id_roles_providers_oidc_manual.html) and
    # [Federation Through a Web-based Identity
    # Provider](https://docs.aws.amazon.com/IAM/latest/UserGuide/id_credentials_temp_request.html#api_assumerolewithwebidentity).
    #
    # - [Amazon Web Services SDK for iOS Developer Guide](http://aws.amazon.com/sdkforios/) and
    # [Amazon Web Services SDK for Android Developer Guide](http://aws.amazon.com/sdkforandroid/).
    # These toolkits contain sample apps that show how to invoke the identity providers. The toolkits
    # then show how to use the information from these providers to get and use temporary security
    # credentials.
    ...
    # organization. For steps to enable this feature, see [Centralize root access for member
    # accounts](https://docs.aws.amazon.com/IAM/latest/UserGuide/id_root-enable-root-access.html) in
    # the *IAM User Guide*.
    #
    ...
    # session. For more information, see [Track privileged tasks in
    ...
    # IAM](https://docs.aws.amazon.com/IAM/latest/UserGuide/best-practices.html). In addition, you can
    # use [service control
    ...
    # status message, a user must be granted permissions through an IAM
    # [policy](https://docs.aws.amazon.com/IAM/latest/UserGuide/access_policies.html) to request the
    ...
    # information about access keys, see [Managing Access Keys for IAM
    # Users](https://docs.aws.amazon.com/IAM/latest/UserGuide/id_credentials_access-keys.html) in the
    # *IAM User Guide*.
    #
    # When you pass an access key ID to this operation, it returns the ID of the Amazon Web Services
    # account to which the keys belong. Access key IDs beginning with `AKIA` are long-term credentials
    # for an IAM user or the Amazon Web Services account root user. Access key IDs beginning with
    # `ASIA` are temporary credentials that are created using STS operations. If the account in the
    # response belongs to you, you can sign in as the root user and review your root user access keys.
    # Then, you can pull a [credentials
    # report](https://docs.aws.amazon.com/IAM/latest/UserGuide/id_credentials_getting-report.html) to
    # learn which IAM user owns the keys. To learn who requested the temporary credentials for an
    # `ASIA` access key, view the STS events in your [CloudTrail
    # logs](https://docs.aws.amazon.com/IAM/latest/UserGuide/cloudtrail-integration.html) in the *IAM
    # User Guide*.
    #
    # This operation does not indicate the state of the access key. The key might be active, inactive,
    # or deleted. Active keys might not have permissions to perform an operation. Providing a deleted
    ...
    #
    # No permissions are required to perform this operation. If an administrator attaches a policy to
    # your identity that explicitly denies access to the `sts:GetCallerIdentity` action, you can still
    # perform this operation. Permissions are not required because the same information is returned
    # when access is denied. To view an example response, see [I Am Not Authorized to Perform:
    ...
    # key, and a security token) for a user. A typical use is in a proxy application that gets
    ...
    # IAM user. As a result, this call is appropriate in contexts where those credentials can be
    ...
    # in the *IAM User Guide*.
    #
    # Although it is possible to call `GetFederationToken` using the security credentials of an Amazon
    # Web Services account root user rather than an IAM user that you create for the purpose of a
    # proxy application, we do not recommend it. For more information, see [Safeguard your root user
    ...
    # Cognito](http://aws.amazon.com/cognito/) or `AssumeRoleWithWebIdentity`. For more information,
    # see [Federation Through a Web-based Identity
    # Provider](https://docs.aws.amazon.com/IAM/latest/UserGuide/id_credentials_temp_request.html#api_assumerolewithwebidentity)
    # in the *IAM User Guide*.
    #
    # **Session duration**
    #
    # The temporary credentials are valid for the specified duration, from 900 seconds (15 minutes) up
    # to a maximum of 129,600 seconds (36 hours). The default session duration is 43,200 seconds (12
    # hours). Temporary credentials obtained by using the root user credentials have a maximum
    ...
    # resulting federated user session has no permissions. When you pass session policies, the session
    # permissions are the intersection of the IAM user policies and the session policies that you
    # pass. This gives you a way to further restrict the permissions for a federated user. You cannot
    # use session policies to grant more permissions than those that are defined in the permissions
    # policy of the IAM user. For more information, see [Session
    # Policies](https://docs.aws.amazon.com/IAM/latest/UserGuide/access_policies.html#policies_session)
    # in the *IAM User Guide*. For information about using `GetFederationToken` to create temporary
    # security credentials, see [GetFederationToken—Federation Through a Custom Identity
    ...
    # **Tags**
    #
    # (Optional) You can pass tag key-value pairs to your session. These are called session tags. For
    ...
    # have separate `Department` and `department` tag keys. Assume that the user that you are
    # federating has the `Department`=`Marketing` tag and you pass the `department`=`engineering`
    ...
    #
    # MFA-enabled IAM users must call `GetSessionToken` and submit an MFA code that is associated with
    # their MFA device. Using the temporary security credentials that the call returns, IAM users can
    ...
    # to control authentication operations. For more information, see [Permissions for
    ...
    # security credentials of an IAM user. Credentials that are created by IAM users are valid for the
    # duration that you specify. This duration can range from 900 seconds (15 minutes) up to a maximum
    ...
    #
    # - You cannot call any IAM API operations unless MFA authentication information is included in
    # the request.
    #
    ...
    #
    # Although it is possible to call `GetSessionToken` using the security credentials of an Amazon
    # Web Services account root user rather than an IAM user, we do not recommend it. If
    # `GetSessionToken` is called using root user credentials, the temporary credentials have root
    # user permissions. For more information, see [Safeguard your root user credentials and don't use
    # them for everyday
    # tasks](https://docs.aws.amazon.com/IAM/latest/UserGuide/best-practices.html#lock-away-credentials)
    # in the *IAM User Guide*
    #
