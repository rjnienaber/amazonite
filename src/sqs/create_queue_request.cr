module Amazonite::Sqs
  class CreateQueueRequest
    ...
    # rejects it. Valid values: An integer from 1,024 bytes (1 KiB) to 1,048,576 bytes (1 MiB).
    ...
    # Default: 345,600 (4 days). When you change a queue's attributes, the change can take up to 60
    # seconds for most of the attributes to propagate throughout the Amazon SQS system. Changes made
    # to the `MessageRetentionPeriod` attribute can take up to 15 minutes and will impact existing
    ...
    # policy structure, see [Overview of Amazon Web Services IAM
    # Policies](https://docs.aws.amazon.com/IAM/latest/UserGuide/PoliciesOverview.html) in the *IAM
    # User Guide*.
    #
    ...
    # being moved to the dead-letter queue. Default: 10. When the `ReceiveCount` for a message exceeds
    ...
    #
    # - `byQueue` – Only queues specified by the `sourceQueueArns` parameter can specify this queue as
    ...
    # Amazon SQS or a custom CMK. For more information, see [Key
    ...
    # the alias of a custom CMK can, for example, be `alias/*MyAlias* `. For more examples, see
    ...
    # reuse a [data
    # key](https://docs.aws.amazon.com/kms/latest/developerguide/concepts.html#data-keys) to encrypt
    # or decrypt messages before calling KMS again. An integer representing seconds, between 60
    # seconds (1 minute) and 86,400 seconds (24 hours). Default: 300 (5 minutes). A shorter time
    # period provides better security but results in more calls to KMS which might incur charges after
    ...
    # this attribute, you must also provide the `MessageGroupId` for your messages explicitly.
    #
    ...
    # in the *Amazon SQS Developer Guide*.
    #
    # - `ContentBasedDeduplication` – Enables content-based deduplication. Valid values are `true` and
    # `false`. For more information, see [Exactly-once
    ...
    # in the *Amazon SQS Developer Guide*.
    #
    # To be able to tag a queue on creation, you must have the `sqs:CreateQueue` and `sqs:TagQueue`
    # permissions.
    #
