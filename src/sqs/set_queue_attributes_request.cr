module Amazonite::Sqs
  class SetQueueAttributesRequest
    ...
    # rejects it. Valid values: An integer from 1,024 bytes (1 KiB) up to 1,048,576 bytes (1 MiB).
    ...
    # days). Default: 345,600 (4 days). When you change a queue's attributes, the change can take up
    # to 60 seconds for most of the attributes to propagate throughout the Amazon SQS system. Changes
    ...
    # Policies](https://docs.aws.amazon.com/IAM/latest/UserGuide/PoliciesOverview.html) in the
    ...
    # While the alias of the AWS-managed CMK for Amazon SQS is always `alias/aws/sqs`, the alias of a
    # custom CMK can, for example, be `alias/*MyAlias* `. For more examples, see
    ...
    def_equals_and_hash(@queue_url, @attributes)
  end
end
