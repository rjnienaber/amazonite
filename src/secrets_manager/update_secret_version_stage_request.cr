...
module Amazonite::SecretsManager
  class UpdateSecretVersionStageRequest
    ...
    def validate! : Nil
      ...
      if value = @remove_from_version_id
        raise Core::ValidationError.new("RemoveFromVersionId length must be >= 32") if value.size < 32
        raise Core::ValidationError.new("RemoveFromVersionId length must be <= 64") if value.size > 64
      end

      if value = @move_to_version_id
        raise Core::ValidationError.new("MoveToVersionId length must be >= 32") if value.size < 32
        raise Core::ValidationError.new("MoveToVersionId length must be <= 64") if value.size > 64
      end
    end
