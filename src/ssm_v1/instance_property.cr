  ...
  class InstanceProperty
    ...
    def validate! : Nil
      ...
      if value = @instance_type
        raise Core::ValidationError.new("InstanceType length must be >= 0") if value.size < 0
        raise Core::ValidationError.new("InstanceType length must be <= 120") if value.size > 120
      end

      if value = @instance_role
        raise Core::ValidationError.new("InstanceRole length must be >= 0") if value.size < 0
        raise Core::ValidationError.new("InstanceRole length must be <= 120") if value.size > 120
      end

      if value = @key_name
        raise Core::ValidationError.new("KeyName length must be >= 0") if value.size < 0
        raise Core::ValidationError.new("KeyName length must be <= 255") if value.size > 255
      end

      if value = @instance_state
        raise Core::ValidationError.new("InstanceState length must be >= 0") if value.size < 0
        raise Core::ValidationError.new("InstanceState length must be <= 120") if value.size > 120
      end

      if value = @architecture
        raise Core::ValidationError.new("Architecture length must be >= 0") if value.size < 0
        raise Core::ValidationError.new("Architecture length must be <= 120") if value.size > 120
      end
