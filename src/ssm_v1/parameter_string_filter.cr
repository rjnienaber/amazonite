...
module Amazonite::SsmV1
  ...
  class ParameterStringFilter
    ...
    def validate! : Nil
      ...
      if value = @option
        raise Core::ValidationError.new("Option length must be >= 1") if value.size < 1
        raise Core::ValidationError.new("Option length must be <= 10") if value.size > 10
      end
    ...
    def_equals_and_hash(@key, @option, @values)
  end
end
