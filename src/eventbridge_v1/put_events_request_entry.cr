  ...
  class PutEventsRequestEntry
    ...
    def validate! : Nil
      ...
      if value = @trace_header
        raise Core::ValidationError.new("TraceHeader length must be >= 1") if value.size < 1
        raise Core::ValidationError.new("TraceHeader length must be <= 500") if value.size > 500
      end
    end
