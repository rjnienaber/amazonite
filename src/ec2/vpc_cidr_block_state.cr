...
module Amazonite::EC2
  ...
  class VpcCidrBlockState
    ...
    property state : VpcCidrBlockStateCode | Nil

    # A message about the status of the CIDR block, if applicable.
    property status_message : String | Nil

    def initialize(
      @state : VpcCidrBlockStateCode | Nil = nil,
    ...
    def self.from_xml(node : XML::Node) : self
      new(
        state: (n = node.xpath_node("*[local-name()='state']")) ? AEC::VpcCidrBlockStateCode.from_json_object_key?(n.content) : nil,
