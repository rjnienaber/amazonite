...
module Amazonite::EC2
  class CreateApplicationStatusCheckRequest
    # The health check paths to use for the application status check. Health check paths define the
    # network path from a source subnet to one or more destination subnets for cross-Availability Zone
    # or Availability Zone to Local Zone health checking. If omitted, health checks are performed in
    # the same subnet as the instance.
    ...
    property protocol : NetworkProtocolEnum
    ...
    def self.from_xml(node : XML::Node) : self
      new(
        ...
        protocol: ((n = node.xpath_node("*[local-name()='Protocol']")) ? AEC::NetworkProtocolEnum.from_json_object_key?(n.content) : nil).not_nil!,
        port: Core::XMLValue.i32(node.xpath_node("*[local-name()='Port']")).not_nil!,
    ...
    def_equals_and_hash(@health_check_paths, @aggregation, @protocol, @port, @path, @device_index, @ip_version, @ip_scope, @interval, @timeout, @failure_threshold, @success_threshold, @status_code_matcher, @initialization_grace_period_seconds, @tag_specifications, @client_token, @dry_run)
  end
end
