...
module Amazonite::EC2
  ...
  class RouteTableAssociationState
    ...
    property state : RouteTableAssociationStateCode | Nil

    # The status message, if applicable.
    property status_message : String | Nil

    def initialize(
      @state : RouteTableAssociationStateCode | Nil = nil,
    ...
    def self.from_xml(node : XML::Node) : self
      new(
        state: (n = node.xpath_node("*[local-name()='state']")) ? AEC::RouteTableAssociationStateCode.from_json_object_key?(n.content) : nil,
