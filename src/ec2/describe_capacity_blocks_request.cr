...
module Amazonite::EC2
  class DescribeCapacityBlocksRequest
    ...
    property capacity_block_ids : Array(String) | Nil
    ...
    #
    # - `end-date` - The date and time at which the Capacity Block expires. When a Capacity Block
    ...
    #
    # - `state` - The state of the Capacity Block (`active` | `expired` | `unavailable` | `cancelled`
    # | `failed` | `scheduled` | `payment-pending` | `payment-failed`).
    #
    ...
    def initialize(
      @capacity_block_ids : Array(String) | Nil = nil,
    ...
    def to_query_params(prefix : String) : Array({String, String})
      ...
      (@capacity_block_ids || [] of String).each_with_index(1) do |item, i|
        params << {"#{prefix}CapacityBlockId.#{i}", item}
      end
    ...
    def self.from_xml(node : XML::Node) : self
      new(
        capacity_block_ids: node.xpath_nodes("*[local-name()='CapacityBlockId']/*[local-name()='item']").map { |n| n.content },
    ...
    def_equals_and_hash(@capacity_block_ids, @next_token, @max_results, @filters, @dry_run)
  end
end
