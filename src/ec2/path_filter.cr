...
module Amazonite::EC2
  ...
  class PathFilter
    ...
    property source_port_range : FilterPortRange | Nil

    # The destination IPv4 address.
    property destination_address : String | Nil

    # The destination port range.
    property destination_port_range : FilterPortRange | Nil

    def initialize(
      @source_address : String | Nil = nil,
      @source_port_range : FilterPortRange | Nil = nil,
      @destination_address : String | Nil = nil,
      @destination_port_range : FilterPortRange | Nil = nil,
    )
    end
    ...
    def self.from_xml(node : XML::Node) : self
      new(
        ...
        source_port_range: node.xpath_node("*[local-name()='sourcePortRange']").try { |n| FilterPortRange.from_xml(n) },
        destination_address: Core::XMLValue.string(node.xpath_node("*[local-name()='destinationAddress']")),
        destination_port_range: node.xpath_node("*[local-name()='destinationPortRange']").try { |n| FilterPortRange.from_xml(n) },
      )
    end
