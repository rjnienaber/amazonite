...
module Amazonite::EC2
  ...
  class DisableFastSnapshotRestoreStateErrorItem
    ...
    # The error.
    property error : DisableFastSnapshotRestoreStateError | Nil

    def initialize(
      @availability_zone : String | Nil = nil,
      @availability_zone_id : String | Nil = nil,
      @error : DisableFastSnapshotRestoreStateError | Nil = nil,
    )
    end
    ...
    def self.from_xml(node : XML::Node) : self
      new(
        ...
        error: node.xpath_node("*[local-name()='error']").try { |n| DisableFastSnapshotRestoreStateError.from_xml(n) },
      )
    end
