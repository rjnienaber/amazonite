...
module Amazonite::EC2
  class DescribeSecondaryNetworksRequest
    ...
    #
    # - `ipv4-cidr-block-association.association-id` - The association ID for an IPv4 CIDR block
    # associated with the secondary network.
    #
    # - `ipv4-cidr-block-association.cidr-block` - An IPv4 CIDR block associated with the secondary
    # network.
    #
    # - `ipv4-cidr-block-association.state` - The state of an IPv4 CIDR block associated with the
    # secondary network.
    #
    ...
    #
    # - `state` - The state of the secondary network (`create-in-progress` | `create-complete` |
    # `create-failed` | `delete-in-progress` | `delete-complete` | `delete-failed`).
    #
    ...
    property secondary_network_ids : Array(String) | Nil
    ...
    def initialize(
      ...
      @secondary_network_ids : Array(String) | Nil = nil,
    )
    end
    ...
    def to_query_params(prefix : String) : Array({String, String})
      ...
      (@secondary_network_ids || [] of String).each_with_index(1) do |item, i|
        params << {"#{prefix}SecondaryNetworkId.#{i}", item}
      end
      params
    end
    ...
    def self.from_xml(node : XML::Node) : self
      new(
        ...
        secondary_network_ids: node.xpath_nodes("*[local-name()='SecondaryNetworkId']/*[local-name()='item']").map { |n| n.content },
      )
    end
    ...
    def_equals_and_hash(@dry_run, @filters, @max_results, @next_token, @secondary_network_ids)
  end
end
