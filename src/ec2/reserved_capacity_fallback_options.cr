  ...
  class ReservedCapacityFallbackOptions
    ...
    def to_query_params(prefix : String) : Array({String, String})
      ...
      (@market_types || [] of ReservedCapacityFallbackMarketType).each_with_index(1) do |item, i|
        params << {"#{prefix}MarketTypeSet.#{i}", item.to_json_object_key}
      end
      params
    end
    ...
    def self.from_xml(node : XML::Node) : self
      new(
        market_types: node.xpath_nodes("*[local-name()='marketTypeSet']/*[local-name()='item']").compact_map { |n| AEC::ReservedCapacityFallbackMarketType.from_json_object_key?(n.content) },
      )
    end
