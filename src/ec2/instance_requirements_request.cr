...
module Amazonite::EC2
  ...
  class InstanceRequirementsRequest
    ...
    property v_cpu_count : VCpuCountRangeRequest

    # The minimum and maximum amount of memory, in MiB.
    property memory_mi_b : MemoryMiBRequest
    ...
    property memory_gi_b_per_v_cpu : MemoryGiBPerVCpuRequest | Nil

    # The instance types to exclude.
    #
    # You can use strings with one or more wild cards, represented by an asterisk (`*`), to exclude an
    # instance family, type, size, or generation. The following are examples: `m5.8xlarge`, `c5*.*`,
    # `m5a.*`, `r*`, `*3*`.
    #
    ...
    #
    # Default: No minimum or maximum limits
    property network_interface_count : NetworkInterfaceCountRequest | Nil
    ...
    #
    # Default: No minimum or maximum limits
    property total_local_storage_gb : TotalLocalStorageGBRequest | Nil
    ...
    #
    # Default: No minimum or maximum limits
    property baseline_ebs_bandwidth_mbps : BaselineEbsBandwidthMbpsRequest | Nil
    ...
    #
    # Default: No minimum or maximum limits
    property accelerator_count : AcceleratorCountRequest | Nil
    ...
    #
    # Default: No minimum or maximum limits
    property accelerator_total_memory_mi_b : AcceleratorTotalMemoryMiBRequest | Nil

    # The minimum and maximum amount of baseline network bandwidth, in gigabits per second (Gbps). For
    # more information, see [Amazon EC2 instance network
    # bandwidth](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/ec2-instance-network-bandwidth.html)
    # in the *Amazon EC2 User Guide*.
    #
    # Default: No minimum or maximum limits
    property network_bandwidth_gbps : NetworkBandwidthGbpsRequest | Nil
    ...
    # in the *Amazon EC2 User Guide*.
    property baseline_performance_factors : BaselinePerformanceFactorsRequest | Nil
    ...
    def initialize(
      ...
      @memory_gi_b_per_v_cpu : MemoryGiBPerVCpuRequest | Nil = nil,
      ...
      @network_interface_count : NetworkInterfaceCountRequest | Nil = nil,
      @local_storage : LocalStorage | Nil = nil,
      @local_storage_types : Array(LocalStorageType) | Nil = nil,
      @total_local_storage_gb : TotalLocalStorageGBRequest | Nil = nil,
      @baseline_ebs_bandwidth_mbps : BaselineEbsBandwidthMbpsRequest | Nil = nil,
      @accelerator_types : Array(AcceleratorType) | Nil = nil,
      @accelerator_count : AcceleratorCountRequest | Nil = nil,
      @accelerator_manufacturers : Array(AcceleratorManufacturer) | Nil = nil,
      @accelerator_names : Array(AcceleratorName) | Nil = nil,
      @accelerator_total_memory_mi_b : AcceleratorTotalMemoryMiBRequest | Nil = nil,
      @network_bandwidth_gbps : NetworkBandwidthGbpsRequest | Nil = nil,
      @allowed_instance_types : Array(String) | Nil = nil,
      @max_spot_price_as_percentage_of_optimal_on_demand_price : Int32 | Nil = nil,
      @baseline_performance_factors : BaselinePerformanceFactorsRequest | Nil = nil,
    ...
    def to_query_params(prefix : String) : Array({String, String})
      ...
      params.concat(@v_cpu_count.to_query_params("#{prefix}VCpuCount."))

      params.concat(@memory_mi_b.to_query_params("#{prefix}MemoryMiB."))

      (@cpu_manufacturers || [] of CpuManufacturer).each_with_index(1) do |item, i|
        params << {"#{prefix}CpuManufacturer.#{i}", item.to_json_object_key}
      end
      ...
      (@excluded_instance_types || [] of String).each_with_index(1) do |item, i|
        params << {"#{prefix}ExcludedInstanceType.#{i}", item}
      end

      (@instance_generations || [] of InstanceGeneration).each_with_index(1) do |item, i|
        params << {"#{prefix}InstanceGeneration.#{i}", item.to_json_object_key}
      end
      ...
      (@local_storage_types || [] of LocalStorageType).each_with_index(1) do |item, i|
        params << {"#{prefix}LocalStorageType.#{i}", item.to_json_object_key}
      end
      ...
      (@accelerator_types || [] of AcceleratorType).each_with_index(1) do |item, i|
        params << {"#{prefix}AcceleratorType.#{i}", item.to_json_object_key}
      end
      ...
      (@accelerator_manufacturers || [] of AcceleratorManufacturer).each_with_index(1) do |item, i|
        params << {"#{prefix}AcceleratorManufacturer.#{i}", item.to_json_object_key}
      end

      (@accelerator_names || [] of AcceleratorName).each_with_index(1) do |item, i|
        params << {"#{prefix}AcceleratorName.#{i}", item.to_json_object_key}
      end
      ...
      (@allowed_instance_types || [] of String).each_with_index(1) do |item, i|
        params << {"#{prefix}AllowedInstanceType.#{i}", item}
      end
    ...
    def self.from_xml(node : XML::Node) : self
      new(
        v_cpu_count: node.xpath_node("*[local-name()='VCpuCount']").try { |n| VCpuCountRangeRequest.from_xml(n) }.not_nil!,
        memory_mi_b: node.xpath_node("*[local-name()='MemoryMiB']").try { |n| MemoryMiBRequest.from_xml(n) }.not_nil!,
        cpu_manufacturers: node.xpath_nodes("*[local-name()='CpuManufacturer']/*[local-name()='item']").compact_map { |n| AEC::CpuManufacturer.from_json_object_key?(n.content) },
        memory_gi_b_per_v_cpu: node.xpath_node("*[local-name()='MemoryGiBPerVCpu']").try { |n| MemoryGiBPerVCpuRequest.from_xml(n) },
        excluded_instance_types: node.xpath_nodes("*[local-name()='ExcludedInstanceType']/*[local-name()='item']").map { |n| n.content },
        instance_generations: node.xpath_nodes("*[local-name()='InstanceGeneration']/*[local-name()='item']").compact_map { |n| AEC::InstanceGeneration.from_json_object_key?(n.content) },
        ...
        network_interface_count: node.xpath_node("*[local-name()='NetworkInterfaceCount']").try { |n| NetworkInterfaceCountRequest.from_xml(n) },
        local_storage: (n = node.xpath_node("*[local-name()='LocalStorage']")) ? AEC::LocalStorage.from_json_object_key?(n.content) : nil,
        local_storage_types: node.xpath_nodes("*[local-name()='LocalStorageType']/*[local-name()='item']").compact_map { |n| AEC::LocalStorageType.from_json_object_key?(n.content) },
        total_local_storage_gb: node.xpath_node("*[local-name()='TotalLocalStorageGB']").try { |n| TotalLocalStorageGBRequest.from_xml(n) },
        baseline_ebs_bandwidth_mbps: node.xpath_node("*[local-name()='BaselineEbsBandwidthMbps']").try { |n| BaselineEbsBandwidthMbpsRequest.from_xml(n) },
        accelerator_types: node.xpath_nodes("*[local-name()='AcceleratorType']/*[local-name()='item']").compact_map { |n| AEC::AcceleratorType.from_json_object_key?(n.content) },
        accelerator_count: node.xpath_node("*[local-name()='AcceleratorCount']").try { |n| AcceleratorCountRequest.from_xml(n) },
        accelerator_manufacturers: node.xpath_nodes("*[local-name()='AcceleratorManufacturer']/*[local-name()='item']").compact_map { |n| AEC::AcceleratorManufacturer.from_json_object_key?(n.content) },
        accelerator_names: node.xpath_nodes("*[local-name()='AcceleratorName']/*[local-name()='item']").compact_map { |n| AEC::AcceleratorName.from_json_object_key?(n.content) },
        accelerator_total_memory_mi_b: node.xpath_node("*[local-name()='AcceleratorTotalMemoryMiB']").try { |n| AcceleratorTotalMemoryMiBRequest.from_xml(n) },
        network_bandwidth_gbps: node.xpath_node("*[local-name()='NetworkBandwidthGbps']").try { |n| NetworkBandwidthGbpsRequest.from_xml(n) },
        allowed_instance_types: node.xpath_nodes("*[local-name()='AllowedInstanceType']/*[local-name()='item']").map { |n| n.content },
        ...
        baseline_performance_factors: node.xpath_node("*[local-name()='BaselinePerformanceFactors']").try { |n| BaselinePerformanceFactorsRequest.from_xml(n) },
