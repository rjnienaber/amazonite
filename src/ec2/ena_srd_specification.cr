...
module Amazonite::EC2
  ...
  class EnaSrdSpecification
    ...
    property ena_srd_udp_specification : EnaSrdUdpSpecification | Nil

    def initialize(
      @ena_srd_enabled : Bool | Nil = nil,
      @ena_srd_udp_specification : EnaSrdUdpSpecification | Nil = nil,
    )
    end
    ...
    def self.from_xml(node : XML::Node) : self
      new(
        ...
        ena_srd_udp_specification: node.xpath_node("*[local-name()='EnaSrdUdpSpecification']").try { |n| EnaSrdUdpSpecification.from_xml(n) },
      )
    end
