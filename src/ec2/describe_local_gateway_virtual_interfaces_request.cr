...
module Amazonite::EC2
  class DescribeLocalGatewayVirtualInterfacesRequest
    ...
    #
    # - `local-bgp-asn` - The Border Gateway Protocol (BGP) Autonomous System Number (ASN) of the
    # local gateway.
    #
    ...
    def to_query_params(prefix : String) : Array({String, String})
      ...
      (@local_gateway_virtual_interface_ids || [] of String).each_with_index(1) do |item, i|
        params << {"#{prefix}LocalGatewayVirtualInterfaceId.#{i}", item}
      end
    ...
    def self.from_xml(node : XML::Node) : self
      new(
        local_gateway_virtual_interface_ids: node.xpath_nodes("*[local-name()='LocalGatewayVirtualInterfaceId']/*[local-name()='item']").map { |n| n.content },
    ...
    def_equals_and_hash(@local_gateway_virtual_interface_ids, @filters, @max_results, @next_token, @dry_run)
  end
end
