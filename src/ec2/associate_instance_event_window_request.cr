private alias Core = Amazonite::Core

module Amazonite::EC2
  class AssociateInstanceEventWindowRequest
    # Checks whether you have the required permissions for the action, without actually making the
    # request, and provides an error response. If you have the required permissions, the error
    # response is `DryRunOperation`. Otherwise, it is `UnauthorizedOperation`.
    property dry_run : Bool | Nil

    # The ID of the event window.
    property instance_event_window_id : String

    # One or more targets associated with the specified event window.
    property association_target : InstanceEventWindowAssociationRequest

    def initialize(
      @instance_event_window_id : String,
      @association_target : InstanceEventWindowAssociationRequest,
      @dry_run : Bool | Nil = nil,
    )
    end

    def to_query_params(prefix : String) : Array({String, String})
      params = [] of {String, String}

      if value = @dry_run
        params << {"#{prefix}DryRun", Core::QueryValue.bool(value)}
      end

      params << {"#{prefix}InstanceEventWindowId", @instance_event_window_id}

      params.concat(@association_target.to_query_params("#{prefix}AssociationTarget."))
      params
    end

    def self.from_xml(node : XML::Node) : self
      new(
        dry_run: Core::XMLValue.bool(node.xpath_node("*[local-name()='DryRun']")),
        instance_event_window_id: Core::XMLValue.string(node.xpath_node("*[local-name()='InstanceEventWindowId']")).not_nil!,
        association_target: node.xpath_node("*[local-name()='AssociationTarget']").try { |n| InstanceEventWindowAssociationRequest.from_xml(n) }.not_nil!,
      )
    end

    def validate! : Nil
      if value = @association_target
        value.validate!
      end
    end

    def_equals_and_hash(@dry_run, @instance_event_window_id, @association_target)
  end
end
