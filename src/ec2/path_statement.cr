...
module Amazonite::EC2
  ...
  class PathStatement
    ...
    property packet_header_statement : PacketHeaderStatement | Nil

    # The resource statement.
    property resource_statement : ResourceStatement | Nil

    def initialize(
      @packet_header_statement : PacketHeaderStatement | Nil = nil,
    ...
    def self.from_xml(node : XML::Node) : self
      new(
        packet_header_statement: node.xpath_node("*[local-name()='packetHeaderStatement']").try { |n| PacketHeaderStatement.from_xml(n) },
