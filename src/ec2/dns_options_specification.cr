...
module Amazonite::EC2
  ...
  class DnsOptionsSpecification
    ...
    # Indicates whether to enable private DNS only for inbound endpoints. This option is available
    # only for services that support both gateway and interface endpoints. It routes traffic that
    # originates from the VPC to the gateway endpoint and traffic that originates from on-premises to
    ...
    def to_query_params(prefix : String) : Array({String, String})
      ...
      (@private_dns_specified_domains || [] of String).each_with_index(1) do |item, i|
        params << {"#{prefix}PrivateDnsSpecifiedDomain.#{i}", item}
      end
      params
    end
    ...
    def self.from_xml(node : XML::Node) : self
      new(
        ...
        private_dns_specified_domains: node.xpath_nodes("*[local-name()='PrivateDnsSpecifiedDomain']/*[local-name()='item']").map { |n| n.content },
      )
    end
