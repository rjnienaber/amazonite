...
module Amazonite::EC2
  ...
  class FleetSpotMaintenanceStrategies
    ...
    property capacity_rebalance : FleetSpotCapacityRebalance | Nil

    def initialize(
      @capacity_rebalance : FleetSpotCapacityRebalance | Nil = nil,
    )
    end
    ...
    def self.from_xml(node : XML::Node) : self
      new(
        capacity_rebalance: node.xpath_node("*[local-name()='capacityRebalance']").try { |n| FleetSpotCapacityRebalance.from_xml(n) },
      )
    end
