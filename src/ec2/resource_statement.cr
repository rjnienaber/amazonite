  ...
  class ResourceStatement
    ...
    def to_query_params(prefix : String) : Array({String, String})
      ...
      (@resources || [] of String).each_with_index(1) do |item, i|
        params << {"#{prefix}ResourceSet.#{i}", item}
      end

      (@resource_types || [] of String).each_with_index(1) do |item, i|
        params << {"#{prefix}ResourceTypeSet.#{i}", item}
      end
      params
    end
    ...
    def self.from_xml(node : XML::Node) : self
      new(
        resources: node.xpath_nodes("*[local-name()='resourceSet']/*[local-name()='item']").map { |n| n.content },
        resource_types: node.xpath_nodes("*[local-name()='resourceTypeSet']/*[local-name()='item']").map { |n| n.content },
      )
    end
