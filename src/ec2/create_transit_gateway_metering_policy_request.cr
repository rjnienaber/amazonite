private alias Core = Amazonite::Core

module Amazonite::EC2
  class CreateTransitGatewayMeteringPolicyRequest
    # The ID of the transit gateway for which to create the metering policy.
    property transit_gateway_id : String

    # The IDs of the middlebox attachments to include in the metering policy.
    property middlebox_attachment_ids : Array(String) | Nil

    # The tags to assign to the metering policy.
    property tag_specifications : Array(TagSpecification) | Nil

    # Checks whether you have the required permissions for the action, without actually making the
    # request, and provides an error response. If you have the required permissions, the error
    # response is `DryRunOperation`. Otherwise, it is `UnauthorizedOperation`.
    property dry_run : Bool | Nil

    def initialize(
      @transit_gateway_id : String,
      @middlebox_attachment_ids : Array(String) | Nil = nil,
      @tag_specifications : Array(TagSpecification) | Nil = nil,
      @dry_run : Bool | Nil = nil,
    )
    end

    def to_query_params(prefix : String) : Array({String, String})
      params = [] of {String, String}

      params << {"#{prefix}TransitGatewayId", @transit_gateway_id}

      (@middlebox_attachment_ids || [] of String).each_with_index(1) do |item, i|
        params << {"#{prefix}MiddleboxAttachmentId.#{i}", item}
      end

      (@tag_specifications || [] of TagSpecification).each_with_index(1) do |item, i|
        params.concat(item.to_query_params("#{prefix}TagSpecifications.#{i}."))
      end

      if value = @dry_run
        params << {"#{prefix}DryRun", Core::QueryValue.bool(value)}
      end
      params
    end

    def self.from_xml(node : XML::Node) : self
      new(
        transit_gateway_id: Core::XMLValue.string(node.xpath_node("*[local-name()='TransitGatewayId']")).not_nil!,
        middlebox_attachment_ids: node.xpath_nodes("*[local-name()='MiddleboxAttachmentId']/*[local-name()='item']").map { |n| n.content },
        tag_specifications: node.xpath_nodes("*[local-name()='TagSpecifications']/*[local-name()='item']").map { |n| TagSpecification.from_xml(n) },
        dry_run: Core::XMLValue.bool(node.xpath_node("*[local-name()='DryRun']")),
      )
    end

    def validate! : Nil
      if value = @tag_specifications
        value.each(&.validate!)
      end
    end

    def_equals_and_hash(@transit_gateway_id, @middlebox_attachment_ids, @tag_specifications, @dry_run)
  end
end
