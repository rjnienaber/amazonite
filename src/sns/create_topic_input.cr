...
module Amazonite::Sns
  ...
  class CreateTopicInput
    ...
    # endpoints.
    #
    # - `DisplayName` – The display name to use for a topic with SMS, `email`, and `email-json`
    # subscriptions. For `email` and `email-json` subscriptions, the display name is used as the
    # sender name for regular notification messages. Subscription confirmation and unsubscribe
    # confirmation emails always use "Amazon Web Services Notifications" as the sender name.
    #
    # - `MaximumMessageSize` – The maximum size, in bytes, of a message that can be published to the
    # topic. Valid values are `1024` to `1048576` (1 MiB). The default is `262144` (256 KiB).
    #
    # A topic with a `MaximumMessageSize` above 256 KiB must have 100 or fewer subscriptions, and each
    # subscription must be an Amazon SQS, Amazon Data Firehose, or Lambda subscription.
    #
    ...
    # Amazon SNS message data protection is no longer available to new customers. For more information
    # and guidance on alternatives, see [Amazon SNS message data protection availability
