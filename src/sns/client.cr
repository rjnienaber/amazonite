...
module Amazonite::Sns
  class Client < Core::Client
    Log               = ::Log.for("amazonite.sns.client")
    ...
    #
    # By default, the maximum allowed individual message size and the maximum total payload size (the
    # sum of the individual lengths of all of the batched messages) are both 256 KiB (262,144 bytes).
    # To publish larger batches, set the topic's `MaximumMessageSize` attribute, which supports values
    # up to 1 MiB (1,048,576 bytes). The combined size of all messages in the batch, including each
    # message's body and attributes, must not exceed the topic's `MaximumMessageSize`.
    #
