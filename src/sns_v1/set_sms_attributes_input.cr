module Amazonite::SnsV1
  ...
  class SetSMSAttributesInput
    ...
    #
    # `MonthlySpendLimit` – The maximum amount in USD that you are willing to spend each month to send
    # SMS messages. When Amazon SNS determines that sending an SMS message would incur a cost that
    ...
    #
    # By default, the spend limit is set to the maximum allowed by Amazon SNS. If you want to raise
    # the limit, submit an [SNS Limit Increase
    ...
    #
    # `DeliveryStatusIAMRole` – The ARN of the IAM role that allows Amazon SNS to write logs about SMS
    # deliveries in CloudWatch Logs. For each SMS message that you send, Amazon SNS writes a log that
    # includes the message price, the success or failure status, the reason for failure (if the
    ...
    # example, to write logs only for failed deliveries, set this value to `0`. To write logs for 10%
    # of your successful deliveries, set it to `10`.
    #
    ...
    #
    # `UsageReportS3Bucket` – The name of the Amazon S3 bucket to receive daily SMS usage reports from
    # Amazon SNS. Each day, Amazon SNS will deliver a usage report as a CSV file to the bucket. The
    # report includes the following information for each SMS message that was successfully delivered
    ...
    # Activity](https://docs.aws.amazon.com/sns/latest/dg/sms_stats.html) in the *Amazon SNS Developer
    # Guide*.
