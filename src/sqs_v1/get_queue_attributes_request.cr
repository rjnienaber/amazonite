...
module Amazonite::SqsV1
  class GetQueueAttributesRequest
    ...
    #
    # The `AttributeNames` parameter is optional, but if you don't specify values for this parameter,
    ...
    # `ApproximateNumberOfMessages` metrics may not achieve consistency until at least 1 minute after
    # the producers stop sending messages. This period is required for the queue metadata to reach
    ...
    # retains a message. When you change a queue's attributes, the change can take up to 60 seconds
    ...
    # the visibility timeout, see [Visibility
    ...
    #
    # - `KmsMasterKeyId` – Returns the ID of an Amazon Web Services managed customer master key (CMK)
    ...
    # can reuse a data key to encrypt or decrypt messages before calling KMS again. For more
    # information, see [How Does the Data Key Reuse Period
    ...
    # encryption using SQS owned encryption keys. Only one server-side encryption option is supported
    # per queue (for example,
    ...
    #
    # - `FifoQueue` – Returns information about whether the queue is FIFO. For more information, see
    # [FIFO queue
    ...
    # queue. For more information, see [Exactly-once
