class TestHelper
  def initialize
    @use_localstack = !ENV["AWS_PROFILE"]?
  end

  def env
    env = {
      "CRYSTAL_PATH" => "#{Crystal::PATH}:..",
    }
    env["AMAZONITE_DYNAMODB_URL"] = "http://localhost:4566" if @use_localstack
    env["AMAZONITE_SSM_URL"] = "http://localhost:4566" if @use_localstack
    env["AMAZONITE_SQS_URL"] = "http://localhost:4566" if @use_localstack
    env["AMAZONITE_KMS_URL"] = "http://localhost:4566" if @use_localstack
    env["AMAZONITE_SECRETSMANAGER_URL"] = "http://localhost:4566" if @use_localstack
    env["AMAZONITE_LAMBDA_URL"] = "http://localhost:4566" if @use_localstack
    env["AMAZONITE_SNS_URL"] = "http://localhost:4566" if @use_localstack
    env["AMAZONITE_IAM_URL"] = "http://localhost:4566" if @use_localstack
    env["AMAZONITE_MONITORING_URL"] = "http://localhost:4566" if @use_localstack
    env["AMAZONITE_STS_URL"] = "http://localhost:4566" if @use_localstack
    env["AMAZONITE_CLOUDFORMATION_URL"] = "http://localhost:4566" if @use_localstack
    env["AMAZONITE_APIGATEWAY_URL"] = "http://localhost:4566" if @use_localstack
    env["AMAZONITE_EVENTS_URL"] = "http://localhost:4566" if @use_localstack
    env["AMAZONITE_LOGS_URL"] = "http://localhost:4566" if @use_localstack
    env["AMAZONITE_KINESIS_URL"] = "http://localhost:4566" if @use_localstack
    env["AMAZONITE_S3_URL"] = "http://localhost:4566" if @use_localstack
    env["AMAZONITE_EC2_URL"] = "http://localhost:4566" if @use_localstack
    env["AWS_PROFILE"] = "local" unless (ENV["AWS_PROFILE"]? || ENV["AWS_ACCESS_KEY_ID"]?)
    env
  end

  def test_tags
    @use_localstack ? "localstack" : "aws"
  end
end
